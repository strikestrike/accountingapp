	...
	<div class="container-fluid">
		<div class="card project-main pb-0">
			<div class="card-header">
				<button type="submit" class="btn btn-primary btn-square">Add New Template</button>
			</div>
			<div class="card-body">
		...
		<div class="card project-main pb-0">
			<div class="card-header">
				...
				<button type="submit" class="btn btn-primary btn-square">Add New Column</button>
			</div>
			<div class="card-body">
					...
					<div class="col-lg-5 col-md-6 col-sm-6 col-xs-12">
						<div class="from-group mb-3">
							<div class="">
								...
								<select multiple name="city_name" id="city_select" data-live-search="true" class="default-select form-control wide">
				...
				<div class="d-flex justify-content-between" id="file_upload_row">
					...
					<button type="submit" id="saveTempltBtn" class="btn btn-primary btn-square"> Save</button>
				</div>
			</div>
		</div>
		...
		$("#saveTempltBtn").on('click', function() {
			...
			if (comp_cols_id.length == 0) {
				...
				if (template_name) {
					...
					submitTemplate(templtData).then(data => {
						// console.log(data)
						// alert(data)
						if (data == 'success') {
...
<script>
	...
	async function fetchCities(county) {
		...
		const url = '<?= base_url() ?>county/fetchCities'
	...
	function renderCitiesList(cities) {
		...
		if (element) {
			console.log(element);
			cities?.forEach(city => {
				element.append('<option>' + city + '</option>')
			});
			// cities.map((city) => {
			// 	// console.log(city);
			// 	element.append(`<option><span class="c_name">${city}</span></option>`).selectpicker();
			// })
