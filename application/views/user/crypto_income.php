                ...
                <ul class="nav nav-tabs style-2 w-100" id="ListViewTabLink">
                    ...
                    <li class="nav-item">
						<?php if($personalData['steps_completed'] >= 2){ ?>
							<a href="cryptoVerification" class="nav-link">Information Verification</a>
						<?php } else {?>
                        ...
                        <div class="row">
                            <div class="col-xl-12">
                                <div class="project-main2">
                                    <span class="arrow-up-back"></span>
                                    <span class="arrow-up"></span>
                                    <form action="<?= base_url() ?>user/saveCryptoIncome" enctype="multipart/form-data" method="POST">

                                        <div class="row justify-content-between align-items-center">
                                            <?php if (!empty($cryptoIncome)) { ?>
                                            ...
                                            <div class="col-xl-4 col-lg-4 col-sm-6 mb-3 d-flex justify-content-end">
                                                <?php if (!empty($cryptoIncome)) { ?>
                                                    ...
                                                    <?php }elseif($personalData['steps_completed'] >= 2){ ?>
                                                        <a href="cryptoVerification" class="btn btn-primary nextStep"><i class="fas fa-forward"></i> Next</a>
                                                    <?php } ?>
                                                <?php } else { ?>
                                                ...
                                                <div class="col-xl-9 col-lg-9 col-sm-6">
                                                    <?php if (!empty($cryptoIncome)) { ?>
                                                        <div>
                                                            ...
                                                            <h6>VAR Final : <span><?= $cryptoIncome[0]['SUM(var_final)'] ?></span></h6>
                                                        </div>
...
<script>
    window.addEventListener('load', () => {
        ...
        var modules = <?= json_encode($cryptoModulesPrice) ?>;
        // console.log(modules);
        setPrice(modules);
...
<script>
    function setPrice(data='') {
        var countForm = <?= !empty($cryptoCount) ?>;
    ...
    function saveFinalCost() {
        ...
        $.ajax({
            ...
            data: data,
            success : function(response){
                window.location.href = "<?= base_url('user/cryptoVerification') ?>";
            },
            error: function(){
