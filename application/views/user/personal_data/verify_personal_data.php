		...
		<div class="project-nav">
			<div class="card-action card-tabs card-tabs2 w-100">
				<ul class="nav nav-tabs style-2 w-100" id="ListViewTabLink">
					...
					<li class="nav-item">
					<?php if ($personal_data['steps_completed'] >= 1) { ?>
						<a href="<?= base_url() ?>user/step1completed/<?= !empty($personal_data['id']) ? $personal_data['id'] : '' ?>" class="nav-link">Income Type</a>
					<?php } else { ?>
										...
										<form class="<?= !empty($personal_data) ? 'editable' : '' ?>" id="personalDataForm" action="<?= !empty($personal_data) ? base_url() . 'user/editPersonalData'   : base_url() . 'user/savePersonalData' ?>" method="POST">
											...
											<div class="row form-group">
												...
												<div class="col-sm-9">
													<div class="row">
														<div class="col-sm-12">
															<input type="text" maxlength="11" class="form-control numberInput allownumericwithoutdecimal" id="personalData_mobile_number" name="mobile_number" <?= !empty($personal_data['mobile_number']) ? 'value = ' . $personal_data['mobile_number'] : '' ?> placeholder="Mobile Number of 10 digits" required data-maxlen="10">
											...
											<div class="row form-group">
												...
												<div class="col-sm-9">
													<div class="row">
														<div class="col-sm-6">
															<input type="text" class="form-control" id="personalData_address_city" name="city" value="<?= $personal_data['city'] ?? '' ?>" <?= !empty($personal_data['city']) ? 'value = ' . $personal_data['city'] : '' ?> placeholder="City" required>
														</div>
														<div class="col-sm-6 mt-2 mt-sm-0">
												...
												<div class="col-sm-10">
													...
													<?php if (!empty($personal_data)) { ?>
														...
														if ($personal_data['steps_completed'] >= 1) { ?>
															<a href=" <?= base_url() ?>user/step1completed/<?= !empty($personal_data['id']) ? $personal_data['id'] : '' ?>" class="btn btn-primary proceedNext"><i class="fas fa-forward"></i> Next</a>
													<?php }
													} ?>
												</div>
											</div>
		...
		$(".proceedPersonalData").click(() => {
			Swal.fire({
				...
				if (result.value) {
					...
					$.ajax({
						url: '<?= base_url("user/finalSubmitPersonalData") ?>',
						method: "POST",
						data: data,
						success: function(response) {
							Swal.fire(
								...
								.then(function() {
									window.location.href = "<?= base_url() ?>user/step1completed/<?= $personal_data['id'] ?? '' ?>"
								});
							// setTimeout(() => {
